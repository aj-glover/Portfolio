...
// 1. Add grid item to HTML
let html = fs.readFileSync('src/index.html', 'utf8');
const gridItem = '<a href="#preview-18" class="grid__item pos-18" data-title="WHALE Members YouTube"><div class="grid__item-img" style="background-image:url(img/thumbs/18.svg);"></div></a>';
html = html.replace('</div>\n\t\t\t\t<div class="preview">', gridItem + '\n\t\t\t\t</div>\n\t\t\t\t<div class="preview">');

// 2. Add detail page (insert before the closing .preview and .content tags)
const detailPage = '\t\t\t\t\t<div class="preview__item" id="preview-18">\n' +
...
// Insert detail page before the closing .preview and .content divs
html = html.replace('\t\t\t\t</div>\n\t\t\t</div>\n\t\t</main>', detailPage + '\t\t\t\t</div>\n\t\t\t</div>\n\t\t</main>');

fs.writeFileSync('src/index.html', html);
console.log('HTML updated with grid item and detail page');

// 3. Update sync-galleries.js loop from 16 to 18
let syncJs = fs.readFileSync('sync-galleries.js', 'utf8');
syncJs = syncJs.replace('i <= 17', 'i <= 18');
fs.writeFileSync('sync-galleries.js', syncJs);
console.log('sync-galleries.js updated to scan 18 galleries');

// 4. Add .nojekyll and build
